...
bool isAdbAvailableInSystem()
{
    ...
    for (const QString &candidate : candidatePaths) {
        if (!normalizeAdbCandidate(candidate).isEmpty()) {
            return true;
        }
    }
    return false;
...
bool Dialog::ensureAdbAvailable()
{
    if (isAdbAvailableInSystem()) {
        m_hasShownAdbWarning = false;
        return true;
    }

    const QString message = tr("ADB was not found. Please configure the ADB environment variable or add ADB to PATH, then restart QtScrcpy.");
    if (!m_hasShownAdbWarning) {
        outLog(message);
        QMessageBox::warning(this, windowTitle(), message, QMessageBox::Ok);
        m_hasShownAdbWarning = true;
    }
    return false;
}
